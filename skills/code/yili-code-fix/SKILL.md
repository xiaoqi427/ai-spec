...
遗漏逻辑的来源文件路径
    │
    ├── yldc-.../src/com/ibm/gbs/**/*.java  → 后端 Bug → 修复
    ├── yldc-.../WebRoot/**/*.jsp            → 前端 Bug → 标注跳过
    ├── yldc-.../WebRoot/**/*.js             → 前端 Bug → 标注跳过
    └── 同时涉及前后端                        → 只修后端, 前端标注
```

**前端 Bug 输出**:
```markdown
**[前端 Bug]** 对应老代码 `WebRoot/newPages/efinance/claim/T{XXX}/xxx.jsp` L{行号}
遗漏逻辑: {描述}
新框架不修改, 需前端开发处理
```
