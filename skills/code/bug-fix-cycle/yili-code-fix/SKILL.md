...
此 skill 不需要额外安装外部 skill，但**要求运行环境**:


| 依赖    | 版本要求       | 说明                               |
| ------- | -------------- | ---------------------------------- |
| **JDK** | **21（必须）** | 项目强制 Java 21，低版本编译不通过 |
| Maven   | 3.9.6+         | 构建工具                           |
...
# 编译修改的模块（要求 JDK 21）
mvn compile -pl claim-{module}/claim-{module}-service -am -T 1C -Dmaven.repo.local=/Users/xiaoqi/.m2/yili-repository
...
6. **编译必须通过**: 修复后必须编译验证
7. **不要有内部类**: 修复的代码中不要有内部类，内部类会增加代码的复杂性，不利于理解和维护
8. 如非新公共代码和老公共逻辑一致，谨慎修改公共逻辑
9. 修改代码需要备注修复原因和老代码地址
10. 前端问题的直接备注，后面转给前端，把方案写清楚
11. 老代码是前端问题就前端修改，后端问题就后端修改，后端不需要修改的备注好前端需要修改啥，问题和方案是啥
12. lombok是有的哈，使用lombok注解
13. 不要加前端清空行逻辑，不要改前端代码，备注就好，不要改voucher代码，备注就好
