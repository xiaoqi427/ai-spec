---
name: ui-smoke-test
description: UI 冒烟和回归验证工具，支持 SIT 报账单验证、Bug 修复回归、样本自动回退、本地前端启动后的页面检查、截图与 API 证据留存。基于 agent-browser、browser-use、Playwright 和本地脚本实现。适用于 SIT 验证、前端本地联调、登录态复用、无头快速探测和 Bug 修复效果确认。
allowed-tools: Bash(agent-browser:*), Bash(npx agent-browser:*), Bash(browser-use:*), Bash(npx:playwright), Bash(node:*), Bash(python3:*), Bash(yarn:*)
---
# UI Smoke Test (SIT 与本地前端冒烟验证)
@author: sevenxiao
...
1. **SIT 环境回归**：报账单做单、Bug 修复验证、截图取证、样本回退。
2. **本地前端联调**：本地 `yarn dev` 启动后的页面加载、关键元素检查、无头快速探测。
...
6. **`sample-hunt`** - 从 Coding/离线材料/数据库回退样本单号
7. **`api-probe`** - 基于保存的登录态做无头快速接口验证
8. **`local-start-test`** - 本地前端启动后做页面级快速检查
...
- `ui-smoke-test` 被 `bug-fix-pipeline` 调用时，先判断是 **SIT 验证** 还是 **本地前端验证**
- SIT 修复验证优先走 `sit_api_probe.mjs` 这种 API-first 快速链路；只有 API 无法证明时才打开页面
- 本地前端验证优先启动 `fssc-web` 的 dev server，再用 `fast_page_probe.mjs` 或 Playwright 做页面检查
- 浏览器型交互默认先走 `agent-browser`
...
```bash
# 对指定报账单类型做冒烟测试
/skill ui-smoke-test T044

# 指定 SIT 页面 URL 做冒烟测试
/skill ui-smoke-test --url http://pri-fssc-web-sit.digitalyili.com/#/claim/T044

# 验证某个 Bug 修复后的场景
/skill ui-smoke-test --bug 4992

# 对多个报账单类型批量冒烟测试
/skill ui-smoke-test T044 T047 T051

# 执行场景测试（新建→保存→提交）
/skill ui-smoke-test T044 --scenario create-save-submit

# 本地前端启动后做快速检查
/skill ui-smoke-test --url http://localhost:8080/#/claim/T045 --local
```
...
- `playwright.config.ts` 默认 `baseURL` 也是 `http://localhost:8080`
- `vue.config.js` 当前将 `^/api` 代理到 `http://pri-fssc-api-sit.digitalyili.com/`
...
如果要连本地后端，再按项目需要改 `vue.config.js` 代理。
...
### 0. API-first 快速验证（SIT 修复验证优先）

适用场景:

- 已知 Bug 单号，需要快速判断修复是否生效
- 登录态不稳定，不想每次重复打开完整页面
- 验证重点在 `load / newClaimLine / submit / updateClaimLine` 这些接口

典型流程:

```bash
# 1. 把现有 cookie 转成 Playwright storageState
node ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/build_playwright_state.mjs \
  --source-json /Users/xiaoqi/Documents/work/yili/config/coding-cookies.json \
  --output /tmp/coding-state.json

# 2. 提取 Bug 样本
python3 ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/bug_candidate_search.py \
  --bug-id 4919 \
  --output /tmp/4919-candidates.json

# 3. 循环验证，样本不对就自动换
python3 ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/verification_loop.py \
  --bug-id 4919 \
  --candidates-json /tmp/4919-candidates.json \
  --probe-command "node ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/sit_api_probe.mjs \
    --state /tmp/coding-state.json \
    --base-url https://pri-fssc-web-sit.digitalyili.com \
    --flow resolve-load \
    --target-endpoint /api/claim/otc/T045/load \
    --claim-no {claim_no} \
    --classify-js \"lastText.includes('flowId') && !lastText.includes('null') ? 'pass' : 'sample_mismatch'\""
```
...
- 需要把临时验证沉淀成正式回归脚本
- 需要稳定断言、报告和可重复执行能力
- 本地前端启动后，需要稳定地验证首页、列表页、明细页是否能加载

## 认证策略

当需要复用真实浏览器登录态时，按优先级依次尝试：

### 预先生成 Playwright `storageState`

SIT 接口验证和本地 Playwright 都优先复用保存后的状态，不再每次登录：

```bash
# 从已有 cookie 文件生成 storageState
node ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/build_playwright_state.mjs \
  --source-json /Users/xiaoqi/Documents/work/yili/config/coding-cookies.json \
  --output /tmp/coding-state.json

# 或直接从已登录 Chrome 抓当前状态
node ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/build_playwright_state.mjs \
  --from-live-chrome \
  --output /tmp/coding-state.json
```
...
当 Bug 单上给的样本不够，或者当前样本无法打到修复分支时，按这个顺序找：

1. Coding 详情/评论里的明确单号
2. `bug-prefetch` 离线材料
3. `sit-execution-log` 和 `sit-manual-checklist`
4. Oracle 中相同模板、相同或相近 `item2Id` 的样本
...
python3 ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/bug_candidate_search.py \
  --bug-id 4919 \
  --prefetch-root /Users/xiaoqi/Documents/work/yili/yili-out/bug-prefetch \
  --db-config /Users/xiaoqi/Documents/work/yili/ai-spec/skills/code/bug-fix-cycle/db-query/config/db-connection.yaml \
  --db-env sit \
  --output /tmp/4919-candidates.json
```

如果 DB 配置不存在，先只用本地材料，不要阻塞。

---

### local-start-test (本地前端启动测试)

适用场景:

- 改了 `fssc-web` 或 `fssc-web-framework`，需要看页面能否起得来
- 不想先跑完整 E2E，只要确认页面不是白屏、关键元素在
...
```bash
node ai-spec/skills/code/bug-fix-cycle/ui-smoke-test/scripts/fast_page_probe.mjs \
  --state /tmp/coding-state.json \
  --base-url http://localhost:8080 \
  --url http://localhost:8080/#/claim/T045 \
  --wait-text 查询
```

如果本地不需要认证，可以直接传一个空的或最小的 `storageState` 文件。
...
1. **验证优先级**: `sit_api_probe.mjs` → `fast_page_probe.mjs` → `agent-browser` → `browser-use` → `playwright-e2e-testing`
2. **认证优先级**: Playwright `storageState` / Chrome Debug (`--connect`) → Cookie 文件 → Chrome Profile → 账号密码
3. **截图存证**: 每个关键步骤必须截图
4. **不猜测路由**: 路由从代码或路由解析脚本推导，不要猜
5. **超时处理**: 页面加载超时默认 60 秒
6. **本地前端验证**: 先确认 dev server 已启动，再做探测
7. **环境边界**: 不要在生产环境执行测试
8. **测试数据**: 使用测试账号和测试数据，不要影响正式数据

## 参考

- SIT 页面路由: [references/sit-pages.md](references/sit-pages.md)
- 测试场景模板: [references/test-scenarios.md](references/test-scenarios.md)
- 样本排序算法: [references/algorithm.md](references/algorithm.md)
- DB 回退策略: [references/db-fallback.md](references/db-fallback.md)
- Probe 协议: [references/probe-contract.md](references/probe-contract.md)
- 认证配置模板: [config/sit-auth.yaml.example](config/sit-auth.yaml.example)
- 状态转换脚本: `scripts/build_playwright_state.mjs`
- 样本提取脚本: `scripts/bug_candidate_search.py`
- 循环验证脚本: `scripts/verification_loop.py`
- SIT API probe: `scripts/sit_api_probe.mjs`
- 快速页面 probe: `scripts/fast_page_probe.mjs`
