...
description: Bug 修复后的代码提交流程。先 pull 更新，检测冲突，展示 diff，等待用户确认后再 commit + push。当用户修复完 Bug 需要提交代码时使用。
...
---
⚠️ 请确认以上变更是否正确，确认后将执行 git add + commit + push。
```
...
6. **不 force push**: 绝不使用 `git push --force`
