...
# 指定提交信息
/skill git-commit-push "fix(config): #4686 修复会计分组座席状态修改人字段不同步"

# 指定模块目录
/skill git-commit-push fssc-config-service "fix(config): #4686 修复座席状态修改人字段"
```
...
**提交信息**: <type>(<scope>): #<coding号> <subject>
...
git commit -m "<type>(<scope>): #<coding号> <subject>"
...
fix(config): #4686 修复会计分组座席状态修改人字段不同步

修改座席状态时同时更新 MODIFY_* 和 MODIFIER_* 两套字段，
...
**Coding 号（强制）**:
- 每次提交**必须**包含 Coding 缺陷/需求号，格式为 `#<数字>`
- Coding 号放在 scope 之后、subject 描述之前
- Coding 号来源于 Coding 平台（yldc.coding.yili.com）的缺陷或需求编号
- 如果用户未提供 Coding 号，**必须向用户询问**，不得省略
- 一次提交关联多个 Coding 号时用空格分隔：`#4919 #5200`
...
```
fix(t045): #4919 validate product cost amount sign
```

```
fix(common): #5093 #5200 修复现金流名称和删除单据校验
```
...
- [ ] 提交信息包含 Coding 缺陷/需求号（#xxxx）

### 构建检查（如适用）

- [ ] JDK 版本为 21（`java -version` 确认）
...
5. **提交信息规范**: 遵循 `<type>(<scope>): #<coding号> <subject>` 格式
6. **Coding 号强制**: 提交信息**必须**包含 Coding 缺陷/需求号，缺失时必须向用户询问
7. **不 force push**: 绝不使用 `git push --force`（仅 `--force-with-lease` 在错误恢复时允许）
8. **不跳过 hooks**: 绝不使用 `--no-verify`
9. **不提交敏感信息**: 提交前必须检查是否包含密码、Token、密钥
