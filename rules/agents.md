...
* 工作流：ClaimProcessEnum枚举记得用
* Do servcie extends IBaseDoXService
* mapper extends BaseMapperX
* DoServiceImpl extends BaseDoXService
...
<skill>
<name>transform-claim</name>
<description>报账单老代码迁移转换器。用于将财务共享服务中心各类报账单模块的老代码迁移转换到新框架的Service中。支持模块: TR/OTC/PTP/EER/FA/RTR，提供10步标准化迁移流程（New头/行、Save头/行、Delete头/行、Load、Callback、Submit、Load行）。当用户需要迁移老报账单代码到新框架时使用，例如："迁移T044的所有Service到新框架"、"分析T047的老代码并转换到新Service"。详见 /ai-spec/skills/code/transform-claim/skill.md</description>
<location>project</location>
</skill>
